import math # need sqrt(quare root)

# a Point is a Cartesion point (x, y)
# all calues are float unless otherwise stated
class Point(object):
	def __init__(self, x_param = 0.0, y_param = 0.0):
		''' Crate x and y attributes. Default are 0.0'''
		self.x = x_param
		self.y = y_param
	...
	def sum(self, param_pt):
		...
		return Point(self.x + para,_pt.x, self.x + param_pt.x)
